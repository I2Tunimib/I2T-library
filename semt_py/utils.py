...
class Utility:
    ...
    def get_utils_parameters(self, util_name: str) -> str:
        ...
        parameter_info = {
            ...
            "parse_json": {
                ...
                "example_values": {
                    "json_data": '[{"th1": {"label": "col1"}, ...}, {"td1": {"label": "val1"}, ...}]'
                },
            },
            "create_temp_csv": {
            ...
            "create_zip_file": {
                "parameters": {"df": "pd.DataFrame", "zip_filename": "Optional[str]"},
    ...
    def push_to_backend(
        self, dataset_id: str, table_id: str, payload: Dict, debug: bool = False
    ) -> Tuple[str, Dict]:
        """
        ...
        # Push to backend
        backend_url = urljoin(
            self.api_url, f"api/dataset/{dataset_id}/table/{table_id}"
        )
    ...
    def create_zip_file(
        self, df: pd.DataFrame, zip_filename: Optional[str] = None
    ) -> str:
        """
    ...
    def display_json_table(json_table, number_of_rows=None, from_row=0, labels=None):
                ...
                if cell_metadata:
                    ...
                    for meta in cell_metadata:
                        ...
                        if "name" in meta:
                            if isinstance(meta["name"], dict):
                                ...
                                metadata_lines.append(
                                    f"<strong>Name:</strong> {meta['name']}<br>"
                                )
                        ...
                        if "type" in meta and isinstance(meta["type"], list):
                            types = ", ".join(
                                t.get("name", "N/A") for t in meta["type"]
                            )
        ...
        columns_to_keep = labels + [
            f"{label}_metadata" for label in columns_with_metadata
        ]
        ...
        )  # Allow full display of cell contents
